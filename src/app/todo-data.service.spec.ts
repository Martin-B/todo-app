...
import {Todo} from './todo'; // ?? not the other way around?
...
describe('TodoDataService', () => {
  ...
  describe('#getAllTodos()', () => {
    ...
    it('should return all todos', inject([TodoDataService], (service: TodoDataService) => {
      // Create & add a few todo's
      let todo1 = new Todo({title: 'Hello 1', complete: false});
      let todo2 = new Todo({title: 'Hello 2', complete: true});
      service.addTodo(todo1);
      service.addTodo(todo2);
      //  validate with get all todo's
      expect(service.getAllTodos()).toEqual([todo1, todo2]);
    }));

  });// Get all todo's

  // Save & check a few todos, one by one
  describe('#save(todo)', () => {
  ...
  // Populate, check, delete by id and check remaining by id
  describe('#deleteTodoById(id)', () => {
    ...
    // Populate, check, delete an odd ID & check integrity again
